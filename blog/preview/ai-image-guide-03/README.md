...
얼굴 영역을 덮는 마스크를 만든다. ComfyUI의 MaskEditor를 쓰거나, 외부 이미지 편집기에서 흰색 영역으로 칠해도 된다. 핵심은 얼굴 전체를 넉넉하게 덮되, 몸이나 배경은 최소한으로 포함하는 것이다.

마스크가 너무 작으면 얼굴 일부만 바뀌어서 이음새가 생긴다. 너무 크면 주변까지 불필요하게 변경된다. 얼굴 윤곽보다 10~20% 정도 여유를 두는 것이 적당하다.
...
|:---:|:---:|:---:|:---:|
| ![base](images/ep3_base.png) | ![dn03](images/inpaint_dn03.png) | ![dn05](images/inpaint_dn05.png) | ![dn10](images/inpaint_dn10.png) |

> **설정**: Realistic Vision V5.1 | 512x768 | DPM++ SDE Karras | Steps 20 | CFG 7.0 | Seed 300 → 인페인팅 Seed 42

원본은 전신 사진에서 전형적으로 나타나는 뭉개진 얼굴이다. 0.3에서는 변화가 미미하고, 0.5에서 윤곽이 살아나기 시작하며, 1.0에서는 완전히 다른 얼굴이 된다.
...
|:---:|:---:|:---:|:---:|
| ![base](images/ep3_base.png) | ![fd03](images/facedetailer_dn03.png) | ![fd05](images/facedetailer_dn05.png) | ![fd07](images/facedetailer_dn07.png) |

> **설정**: Realistic Vision V5.1 | guide_size 384 | DPM++ SDE Karras | Steps 20 | CFG 7.0 | Seed 42 | face_yolov8m | 원본 Seed 300
...
| 2화 결과 (OpenPose, 보정 전) | 3화 결과 (FaceDetailer 적용) |
|:---:|:---:|
| ![ep2](images/series_ep2_openpose.png) | ![ep3](images/series_ep3_facedetail.png) |

1화부터의 진행 비교:

| 1화 (txt2img) | 2화 (OpenPose) | 3화 (FaceDetailer) |
|:---:|:---:|:---:|
| ![ep1](images/series_ep1_txt2img.png) | ![ep2](images/series_ep2_openpose.png) | ![ep3](images/series_ep3_facedetail.png) |

1화에서 포즈를 잡을 수 없었고, 2화에서 ControlNet으로 포즈를 대충 잡았고, 3화에서 얼굴 디테일을 개선했다. 전신 이미지에서 얼굴이 차지하는 비율이 작아서 극적인 차이는 아니지만, 확대하면 눈과 피부의 선명도가 달라진 것을 확인할 수 있다.
...
*이전: [2화. 구도와 포즈 — ControlNet 입문](../ai-image-guide-02/README.md)*
