...
|:---:|:---:|:---:|
| ![before](images/before_swap.png) | ![source](images/face_source.jpg) | ![after](images/swap_codeformer_05.png) |
...
|:---:|:---:|:---:|
| ![raw](images/swap_raw.png) | ![gfpgan](images/swap_gfpgan.png) | ![cf05](images/swap_codeformer_05.png) |
...
|:---:|:---:|:---:|
| ![cf03](images/swap_codeformer_03.png) | ![cf05](images/swap_codeformer_05.png) | ![cf10](images/swap_codeformer_10.png) |
...
애니메 스타일 이미지에 실사 얼굴을 교체하면, 얼굴만 반실사(semi-realistic)가 된다. 몸은 애니메인데 얼굴만 사진처럼 보이는 언캐니 밸리가 발생한다.

이 문제를 줄이는 방법:

1. **반실사 모델 사용**: DreamShaper처럼 애니메와 실사 중간 스타일의 모델을 쓰면 격차가 줄어든다.
2. **face_restore_visibility 조절**: 1.0이 아닌 0.7~0.8로 낮추면 원본 스타일이 일부 유지된다.
3. **img2img로 후처리**: 교체 후 전체 이미지를 낮은 denoise(0.2~0.3)로 img2img 처리하면 스타일이 통일된다. 단, 얼굴 특징이 약해질 수 있다.

완전히 해결하기는 어렵다. 스타일이 일관된 결과를 원한다면, 참조 얼굴과 생성 이미지의 스타일을 최대한 맞추는 것이 가장 현실적이다.
