...
정식 CMMI 평가가 아니라 도구의 기능 수준에 기반한 참고용 판단이다.
...
**도구 간 파편화**: 도구마다 다루는 영역이 다르고, 통합 표준이 없다. **검증의 한계**: 대부분의 검증이 정적 분석에 머물러 있다. 스펙과 구현의 의미적 정합성을 기계적으로 검증하는 도구는 아직 없다. **규칙 형식 파편화**: CLAUDE.md, .cursor/rules, AGENTS.md, .kiro/steering 등 에이전트마다 형식이 다르다. **현실적 복잡도 미반영**: 대부분의 도구가 단일 저장소, 단일 프로젝트를 전제로 설계되어 있다. FE/BE 분리, 마이크로서비스, 스펙 비대화, 테스트 전략 정의, 모노레포/멀티레포 같은 현실적 문제를 다루는 도구는 아직 없다.
...
하네스 엔지니어링은 "에이전트가 더 똑똑해지면 해결될 문제"가 아니다. 에이전트가 똑똑해질수록, 그 능력을 올바른 방향으로 쓰게 하는 하네스의 중요성은 더 커진다.
