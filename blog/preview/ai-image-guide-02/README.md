...
1. **ControlNet 모델 파일** (.safetensors)
...
| 모델 | 용도 | 파일명 |
|------|------|--------|
| OpenPose | 인체 포즈 제어 | `control_v11p_sd15_openpose.safetensors` |
| Canny | 윤곽선 유지 | `control_v11p_sd15_canny.safetensors` |
| Depth | 깊이/원근 제어 | `control_v11f1p_sd15_depth.safetensors` |

[lllyasviel/ControlNet-v1-1](https://huggingface.co/lllyasviel/ControlNet-v1-1/tree/main) 저장소에서 다운로드한다. 각 파일은 약 1.45GB다.
...
|:---:|:---:|:---:|
| ![e03](images/cmp_endpct_03.png) | ![e08](images/cmp_endpct_08.png) | ![e10](images/cmp_endpct_10.png) | 포즈는 디노이징 초반 스텝에서 이미 결정되기 때문에, 후반부를 풀어줘도 포즈에는 영향이 없고, 세부 디테일 차이도 눈에 띄지 않는다.

처음에는 기본값 1.0으로 두고 신경 쓰지 않아도 된다. 나중에 특정 이미지에서 디테일이 경직된 느낌이 들면 그때 0.8로 낮춰보는 정도로 충분하다.
...
- **End Percent를 0.8**로 두면 디테일이 자연스럽다
- 참조 이미지는 **배경 단순, 인물 한 명, 관절 노출**이 기본
- ControlNet과 체크포인트의 **SD 버전을 일치**시켜야 한다

txt2img에서는 운에 맡기던 포즈를, ControlNet으로 의도한 대로 잡을 수 있게 됐다. 하지만 생성된 이미지의 얼굴이 마음에 들지 않을 수 있다. 다음 화에서는 얼굴을 교체하는 방법을 다룬다.

---

*이전: [1화. 첫 생성 — 프롬프트만 넣으면 되는 거 아니야?]*
*다음: [3화. 얼굴 교체 — AI + 수작업 하이브리드]*
