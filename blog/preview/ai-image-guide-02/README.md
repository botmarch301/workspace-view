...
| 참조 사진 | OpenPose 포즈맵 | 생성 결과 |
|:---:|:---:|:---:|
| ![crane_ref](images/series_crane_reference.jpg) | ![crane_map](images/series_crane_posemap.png) | ![ep2](images/series_ep2_openpose.png) |
...
1화에서 프롬프트만으로는 학다리 자세가 나오지 않았지만, 참조 사진의 포즈를 OpenPose로 지정하니 한 발로 서서 무릎을 올린 자세가 재현됐다. 태권도복도 대략적으로 나온다.

하지만 아직 완벽하지는 않다. 얼굴이 마음에 들지 않을 수 있고, 태권도복의 디테일도 부족하다. 포즈는 잡혔으니, 다음 단계는 이런 세부 요소를 다듬는 것이다.
