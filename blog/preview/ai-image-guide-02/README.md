...
세 결과를 비교하면 차이가 분명하다. OpenPose는 깔끔한 스튜디오 배경에 프롬프트대로 생성됐고, Canny는 참조 사진의 형태에 가장 종속적이며, Depth는 그 중간이다.

**포즈 제어가 목적이라면 OpenPose가 적합하다.** 관절 위치만 추출하기 때문에, 참조 사진의 형태나 배경에 끌려가지 않고 원하는 캐릭터를 만들 수 있다. 이 시리즈에서는 OpenPose 결과를 기준으로 진행한다.
